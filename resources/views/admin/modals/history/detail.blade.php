        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-5">
                        <!-- Gambar Utama -->
                        <div id="mainImageHistory{{ $lelang->id_lelang }}" class="mb-3">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                @foreach ($lelang->barang->gambar as $img)
                                    <img src="{{ asset('storage/barang/' . $img->gambar) }}"
                                        class="rounded gallery-thumb-history-{{ $lelang->id_lelang }} {{ $img->is_primary ? 'border border-primary border-3' : 'border border-secondary' }}"
                                        style="width: 70px; height: 70px; object-fit: cover; cursor: pointer;"
                                        onclick="changeMainImageHistory{{ $lelang->id_lelang }}('{{ asset('storage/barang/' . $img->gambar) }}', this)"
                                        alt="{{ $lelang->barang->nama_barang }}">
                                @endforeach
                            </div>
                        @endif
                        ...
                        <table class="table table-borderless table-sm">
                            <tbody>
                                    ...
                                    <td>
                                        <span class="badge bg-info">
                                            <img src="{{ asset('assets/icons/checkmark-done-outline.svg') }}"
                                                width="14" height="14" alt="Selesai"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $selisih = $lelang->harga_akhir - $lelang->barang->harga_awal;
                                            $persentase = ($selisih / $lelang->barang->harga_awal) * 100;
                                        @endphp
                                        <span class="badge bg-{{ $selisih > 0 ? 'success' : 'secondary' }}">
                                ...
                                <tr>
                                    <td><strong>Ditutup</strong></td>
                                    <td>{{ $lelang->updated_at->format('d F Y H:i') }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
<script>
    function changeMainImageHistory{{ $lelang->id_lelang }}(src, element) {
        // Ganti gambar utama
        document.querySelector('#mainImageHistory{{ $lelang->id_lelang }} img').src = src;

        // Reset semua border
        document.querySelectorAll('.gallery-thumb-history-{{ $lelang->id_lelang }}').forEach(function(img) {
