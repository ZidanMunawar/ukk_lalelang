    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Detail Lelang - {{ $lelang->barang->nama_barang }}</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-5">
                        <!-- Gambar Utama -->
                        <div id="mainImageDetail{{ $lelang->id_lelang }}" class="mb-3">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                @foreach ($lelang->barang->gambar as $img)
                                    <img src="{{ asset('storage/barang/' . $img->gambar) }}"
                                        class="rounded gallery-thumb-detail-{{ $lelang->id_lelang }} {{ $img->is_primary ? 'border border-primary border-3' : 'border border-secondary' }}"
                                        style="width: 70px; height: 70px; object-fit: cover; cursor: pointer;"
                                        onclick="changeMainImageDetail{{ $lelang->id_lelang }}('{{ asset('storage/barang/' . $img->gambar) }}', this)"
                                        alt="{{ $lelang->barang->nama_barang }}">
                                @endforeach
                            </div>
                        @endif
                    ...
                    <div class="col-md-7">
                        ...
                        <!-- Tombol Tawar untuk lelang yang bisa ditawar -->
                            ...
                            <tbody>
                                @foreach ($lelang->historyLelang->sortByDesc('created_at') as $history)
                                    <tr>
                                        <td>
                                            @auth('masyarakat')
                                                @if ($history->user->id_user == Auth::guard('masyarakat')->id())
                                                    <strong>Anda</strong>
                                                @else
...
<script>
    function changeMainImageDetail{{ $lelang->id_lelang }}(src, element) {
        // Ganti gambar utama
        document.querySelector('#mainImageDetail{{ $lelang->id_lelang }} img').src = src;

        // Reset semua border
        document.querySelectorAll('.gallery-thumb-detail-{{ $lelang->id_lelang }}').forEach(function(img) {
